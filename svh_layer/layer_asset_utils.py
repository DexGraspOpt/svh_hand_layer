    ...
    for root, dirs, files in os.walk(dst):
        for filename in files:
            if filename.endswith('.obj') or filename.endswith('.stl'):
                ...
                if 'palm' in filepath or 'metacarpal' in filepath:
                    tmp_method = 'coacd'
                else:
                    tmp_method = method
                if tmp_method == 'coacd':
                    mesh = coacd.Mesh(mesh.vertices, mesh.faces)
                    parts = coacd.run_coacd(mesh, threshold=threshold, max_convex_hull=4)
                    meshes = []
                    for part in parts:
...
def sample_points_on_mesh(src_dir='../assets/hand_meshes_cvx'):
    for root, dirs, files in os.walk(src_dir):
        for filename in files:
            ...
            np.random.seed(0)
            points, idx = trimesh.sample.sample_surface_even(convex_mesh, 50000)
            point_normals = convex_mesh.face_normals[idx]
            # points, point_normals = pcu.downsample_point_cloud_on_voxel_grid(0.001, points, point_normals)
            point_normals = point_normals / np.linalg.norm(point_normals, axis=1).reshape(-1, 1)
            ...
            if vis:
                ...
                scene = trimesh.Scene([pc, ray_visualization, convex_mesh])
                scene.show()

            info = np.concatenate([points, point_normals], axis=-1)
            dst_path = filepath.replace('hand_meshes_cvx', 'hand_points')
            dst_dir = os.path.dirname(dst_path)
            if not os.path.exists(dst_dir):
                os.makedirs(dst_dir, exist_ok=True)
            np.save(dst_path.replace('.stl', '.npy'), info)


def sample_visible_points(voxel_size=0.004):
    count = 0
    ...
    for root, dirs, files in os.walk(dst):
        for filename in files:
            ...
            distance, v_sampled_idx = KDTree(point_info[:, :3]).query(v_sampled, k=1)
